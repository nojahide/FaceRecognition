...
print("labels(2): ", type(labels), ", ", labels.shape)
print("images(2): ", type(images), ", ", images.shape)

#学習用/評価用に分ける前にimageの配列にファイルindexを結合
fileindexies.reshape(fileindexies.size, -1)
for_split_X = np.column_stack([images, fileindexies])

# split into a training and testing set 学習用/評価用に分ける
X_train, X_test, y_train, y_test = train_test_split(
    for_split_X, labels, test_size=0.25, random_state=42)

# 学習用/評価用に分けたものからファイルindexを分離＆削除
